...
export function AI() {
    ...
    const sendMessageWithLocation = async (msg, latitude, longitude) => {
        ...
        const response = await axios.post('http://13.209.80.79:8001/chat', { message: msg, latitude, longitude }, {headers : {"Authorization": `Bearer ${token}`}});
        stopTypingIndicator();
    ...
    const sendMessage = async (msg, isVoice = false) => {
        ...
        try {
            ...
            const response = await axios.post('http://13.209.80.79:8001/chat', { message: msg }, {headers : {"Authorization": `Bearer ${token}`}});
            stopTypingIndicator();
            handleBotResponse(response.data.response);
        } catch (error) {
            stopTypingIndicator();
