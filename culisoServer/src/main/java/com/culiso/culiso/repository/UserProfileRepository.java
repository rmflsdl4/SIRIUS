...
public interface UserProfileRepository extends JpaRepository<UserEntity, String> {
    @Query("SELECT UserProfileDTO(user.user_id, user.user_name, user.user_nick, user.create_date, user.profile_url) FROM UserEntity user WHERE user.delete_flag = false and user.user_id = 'user1'")
